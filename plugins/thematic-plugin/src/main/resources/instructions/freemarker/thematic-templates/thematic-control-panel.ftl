...
<div class="row" id="main">
    <div class="left_column">
        ...
        <ul class="results">
                <#list searchResults.children.collection as result>
                    ...
                    <li class="rippleLink results_item_${count} selected" onclick="loadPreview('${result.getValue("theme")}', 'results_item_${count}')" >
                        <p class="result_theme">${result.getValue("h1_default")}</p>
                        <p class="result_details">Revenue: $${result.getValue("revenue")}<br>Visits: ${result.getValue("visits")}</p>
                    </li>
                        <#assign count++>
                    <#else>
                    <li class="rippleLink results_item results_item_${count}" onclick="loadPreview('${result.getValue("theme")}', 'results_item_${count}')">
                        <p class=" result_theme">${result.getValue("h1_default")}</p>
                        <p class=" result_details">Revenue: $${result.getValue("revenue")}<br>Visits: ${result.getValue("visits")}</p>
                    </li>
                        <#assign count++>
    ...
    <#--        <div id="spinner" style="position: absolute; top: 45% ; left: 65%; width:10%; height:10%;">
        ...
        <iframe  frameBorder="0"  id="previewPanel" src="${firsturl}" onload="load()" ></iframe>
