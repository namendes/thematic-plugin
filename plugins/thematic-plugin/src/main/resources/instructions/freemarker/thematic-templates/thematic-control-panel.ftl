...
<div class="row" id="main">
    <div class="left_column">
        <form method="get" autocomplete="off">
            <div class="search">
                ...
                <button class="thematic-search" type="submit">
                <#--TODO change to local url-->
                    <img border="0" src="https://cdn1.iconfinder.com/data/icons/TWG_Retina_Icons/24/magnifier.png"/>
                </button>
            </div>
            <ul class="results">
                <#list searchResults.children.collection as result>
                    ...
                    <li class="results_item_${count} selected" onclick="loadPreview('${result.getValue("theme")}', 'results_item_${count}')" >
                        ...
                        <p class="details">Revenue - ${result.getValue("revenue")}<br>Visits - ${result.getValue("visits")}</p>
                    </li>
                        <#assign count++>
                    <#else>
                    <li class=" results_item results_item_${count}" onclick="loadPreview('${result.getValue("theme")}', 'results_item_${count}')">
                        <p>${result.getValue("theme")}</p>
                        <p class="details">Revenue - ${result.getValue("revenue")}<br>Visits - ${result.getValue("visits")}</p>
                    </li>
                        <#assign count++>
    ...
    <#--        <div id="spinner" style="position: absolute; top: 45% ; left: 65%; width:10%; height:10%;">
        ...
        <iframe  frameBorder="0"  id="previewPanel" src="${firsturl}" ></iframe>
